...
(function() {
  ...
  const PDFATSTurbo = {
    ...
    async generateATSPerfectCV(candidateData, tailoredCV, jobData, professionalExperienceKeywords = []) {
      ...
      if (window.PDFDebugPanel) {
        window.PDFDebugPanel.logStart('PDFATSTurbo');
        window.PDFDebugPanel.logInputData(candidateData, tailoredCV);
      }

      try {
        // Parse and format CV content
        const formattedContent = this.formatCVForATS(tailoredCV, candidateData, professionalExperienceKeywords);
        
        // Log parsed sections to debug panel
        if (window.PDFDebugPanel) {
          window.PDFDebugPanel.logParsedSections(formattedContent);
        }
        ...
        if (typeof jspdf !== 'undefined' && jspdf.jsPDF) {
          ...
          if (window.PDFDebugPanel) {
            window.PDFDebugPanel.logWarning('jsPDF not available, using text fallback', 'PDF Generation');
          }
        }
        ...
        // Log output to debug panel
        if (window.PDFDebugPanel) {
          window.PDFDebugPanel.logOutputData(pdfBase64, null, fileName, null);
          window.PDFDebugPanel.logComplete(true);
        }
        ...
        console.error('[PDFATSTurbo] Error generating CV:', error);
        if (window.PDFDebugPanel) {
          window.PDFDebugPanel.logError(error, 'generateATSPerfectCV');
          window.PDFDebugPanel.logComplete(false);
        }
        throw error;
      }
    },
